...
PINGGY_PID_FILE="/dev/shm/v3-obf2-pinggy.pid"
PINGGY_URL_FILE="/dev/shm/v3-obf2-pinggy.url"
PINGGY_LOG="/dev/shm/v3-obf2-pinggy.log"
...
check_root_by_apt() {
  if apt update >/dev/null 2>&1 || sudo apt update >/dev/null 2>&1; then
    ROOTLESS_MODE=0
  else
    ROOTLESS_MODE=1
  fi
...
start_web_dashboard() {
  ...
  if ! pgrep -f "$DASHBOARD_SERVER" >/dev/null 2>&1; then
    nohup env PORT="$DASHBOARD_PORT" VM_ROOT="$SCRIPT_DIR" VM_SCRIPT="$SCRIPT_DIR/v3-obf2.sh" VM_LOG='/dev/shm/v3-obf2-launch.log' VM_PROFILE='/dev/shm/v3-obf2-profile.json' VM_ROOTLESS_FLAG='/dev/shm/v3-obf2-rootless.flag' python3 "$DASHBOARD_SERVER" >/dev/shm/v3-obf2-dashboard.log 2>&1 &
    sleep 1
  fi
}
...
start_http_tunnel() {
  if [[ -f "$PINGGY_PID_FILE" ]] && kill -0 "$(cat "$PINGGY_PID_FILE")" 2>/dev/null; then
    [[ -f "$PINGGY_URL_FILE" ]] && PUBLIC_TUNNEL_URL="$(cat "$PINGGY_URL_FILE" 2>/dev/null || true)"
    return 0
  fi

  if ! command -v ssh >/dev/null 2>&1; then
    warn "ssh không khả dụng để mở Pinggy tunnel"
    return 1
  fi

  local pinggy_cmd=(ssh -p 443 -o StrictHostKeyChecking=no -o ServerAliveInterval=30 -R0:localhost:${DASHBOARD_PORT} free.pinggy.io)
  nohup "${pinggy_cmd[@]}" > "$PINGGY_LOG" 2>&1 &
  echo $! > "$PINGGY_PID_FILE"
  sleep 5
  PUBLIC_TUNNEL_URL=$(grep -m1 -oE 'https://[^[:space:]]+|http://[^[:space:]]+' "$PINGGY_LOG" || true)
  [[ -n "$PUBLIC_TUNNEL_URL" ]] && printf '%s
' "$PUBLIC_TUNNEL_URL" > "$PINGGY_URL_FILE"
  return 0
}
ask() {
  read -rp "$1" ans
...
record_vm_profile() {
  cat > /dev/shm/v3-obf2-profile.json <<EOF
{"os":"$WIN_NAME","cpu_cores":$cpu_core,"ram_gb":$ram_size,"cpu_id":"$(cpu_host_name)","username":"$RDP_USER","password":"$RDP_PASS"}
EOF
}
...
launch_vm() {
  ...
  ram_size="${ram_size:-4}"
  printf '{\"os\":\"%s\",\"cpu_cores\":%s,\"ram_gb\":%s,\"cpu_id\":\"%s\",\"username\":\"%s\",\"password\":\"%s\"}\n' "$WIN_NAME" "$cpu_core" "$ram_size" "$(cpu_host_name)" "$RDP_USER" "$RDP_PASS" > "$SCRIPT_DIR/vm-dashboard/profile.json"
  record_vm_profile
  ...
  if [[ "$use_rdp" == "y" ]]; then
    read -rsp "Nhập Tailscale auth token (Enter để bỏ qua): " TAILSCALE_AUTHKEY
    echo
    ...
    if [[ -n "$TAILSCALE_AUTHKEY" ]]; then
      ...
      warn "Bỏ qua Tailscale auth token"
      TS_IP=""
    fi
  fi
